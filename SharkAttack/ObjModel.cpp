...
void Texture::Bind() {
	...
	stbi_uc* imgData = stbi_load(filepath.c_str(), &width, &height, &bpp, 4);
...
void ObjModel::draw()
{
	...
	glVertexPointer(3, GL_FLOAT, sizeof(Vert), ((float*)modelVerts.data()) + 0);
	glNormalPointer(GL_FLOAT, sizeof(Vert), ((float*)modelVerts.data()) + 3);
	glTexCoordPointer(2, GL_FLOAT, sizeof(Vert), ((float*)modelVerts.data()) + 6);
